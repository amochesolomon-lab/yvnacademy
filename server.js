...
// Setup CORS
const productionOrigin = process.env.SITE_URL ? process.env.SITE_URL.replace(/\/+$/, '') : null;
app.use(cors({
  origin: productionOrigin ? ['http://localhost:3000', 'http://localhost:5173', productionOrigin] : ['http://localhost:3000', 'http://localhost:5173'],
  credentials: true
}));
...
async function initDbSchema(database) {
  ...
  try {
    await database.exec("ALTER TABLE courses ADD COLUMN preview_text TEXT DEFAULT ''");
  } catch (e) {
...
function verifyPassword(password, hashStr) {
  try {
    ...
    return new Promise((resolve, reject) => {
      crypto.scrypt(password, salt, keylen, { N, r, p, maxmem: 128 * 1024 * 1024 }, (err, derivedKey) => {
...
app.get('/api/auth/me', (req, res) => {
  if (req.session.userId) {
    res.json({
      ...
      is_admin: req.session.isAdmin || ADMIN_EMAILS.includes(req.session.userEmail)
    });
  } else {
...
app.get('/api/courses/:id', async (req, res) => {
  ...
  try {
    ...
    let avg_rating = 0;
    if (ratings.length > 0) {
      avg_rating = Math.round((ratings.reduce((sum, r) => sum + r.stars, 0) / ratings.length) * 10) / 10;
    }
...
app.post('/api/courses/:id/buy', loginRequired, upload.single('receipt'), async (req, res) => {
  const courseId = parseInt(req.params.id, 10);
  const userId = req.session.userId;
  if (!req.file) {
  ...
  try {
    ...
    res.json({ message: 'Receipt submitted! We will review and approve your access shortly.' });
  } catch (err) {
    res.status(500).json({ error: 'Database error. Please try again.' });
  }
});
...
app.get('/api/courses/:id/content', loginRequired, async (req, res) => {
  ...
  try {
    ...
    if (!purchase) {
      return res.status(403).json({ error: 'You do not have access to this course yet.' });
    }
    const course = await database.get('SELECT * FROM courses WHERE id=?', [courseId]);
    res.json({ course });
  } catch (err) {
...
app.post('/api/courses/:id/rate', loginRequired, async (req, res) => {
  ...
  try {
    ...
    if (!purchase) {
      return res.status(400).json({ error: 'You can only rate courses you have completed.' });
    }

    // SQLite equivalent of ON CONFLICT(user_id, course_id) DO UPDATE
    const updatedAtExpr = process.env.DATABASE_URL && Pool ? 'NOW()' : "datetime('now')";
    await database.run(`
...
// Approve purchase
app.post('/api/admin/purchases/:id/approve', adminRequired, async (req, res) => {
  const purchaseId = parseInt(req.params.id, 10);
  try {
    const database = await getDbConnection();
    await database.run("UPDATE purchases SET status = 'approved' WHERE id = ?", [purchaseId]);
...
app.post('/api/admin/purchases/:id/reject', adminRequired, async (req, res) => {
  ...
  try {
    const database = await getDbConnection();
    await database.run("UPDATE purchases SET status = ? WHERE id = ?", ['rejected: ' + reason, purchaseId]);
...
app.get('/api/admin/courses', adminRequired, async (req, res) => {
  try {
    const database = await getDbConnection();
    const courses = await database.all('SELECT * FROM courses ORDER BY id DESC');
    res.json(courses);
  } catch (err) {
    res.status(500).json({ error: 'Database error.' });
  }
...
// Add Course
app.post('/api/admin/courses', adminRequired, upload.fields([{ name: 'thumbnail', maxCount: 1 }, { name: 'pdf_file', maxCount: 1 }]), async (req, res) => {
  const { title, instructor, description, price, course_type, telegram_link, preview_text } = req.body;
  if (!title || !instructor || !price) {
    return res.status(400).json({ error: 'Missing title, instructor, or price.' });
  }

  const thumbnailFile = req.files && req.files['thumbnail'] ? fileToDataUrl(req.files['thumbnail'][0]) : '';
  const pdfFile = req.files && req.files['pdf_file'] ? fileToDataUrl(req.files['pdf_file'][0]) : '';
  ...
  try {
    ...
    await database.run(
      `INSERT INTO courses (title, instructor, description, telegram_link, price, course_type, pdf_file, thumbnail, preview_text) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [title, instructor, description, telegram_link || '', price, course_type || 'video', pdfFile, thumbnailFile, preview_text || '']
    );
...
// Edit Course
app.put('/api/admin/courses/:id', adminRequired, upload.fields([{ name: 'thumbnail', maxCount: 1 }, { name: 'pdf_file', maxCount: 1 }]), async (req, res) => {
  const courseId = parseInt(req.params.id, 10);
  const { title, instructor, description, telegram_link, price, is_active, course_type, preview_text } = req.body;
  
  const thumbnailFile = req.files && req.files['thumbnail'] ? fileToDataUrl(req.files['thumbnail'][0]) : null;
  const pdfFile = req.files && req.files['pdf_file'] ? fileToDataUrl(req.files['pdf_file'][0]) : null;
  ...
  try {
    ...
    const updatedInstructor = instructor !== undefined ? instructor : course.instructor;
    const updatedDesc = description !== undefined ? description : course.description;
    const updatedTelLink = telegram_link !== undefined ? telegram_link : course.telegram_link;
    const updatedPrice = price !== undefined ? price : course.price;
    const updatedIsActive = is_active !== undefined ? parseInt(is_active, 10) : course.is_active;
    const updatedType = course_type !== undefined ? course_type : course.course_type;
    const updatedPreview = preview_text !== undefined ? preview_text : course.preview_text;
    ...
    await database.run(`
      ...
      WHERE id=?
    `, [updatedTitle, updatedInstructor, updatedDesc, updatedTelLink, updatedPrice, updatedIsActive,
        updatedThumbnail, updatedType, updatedPdf, updatedPreview, courseId]);
...
app.post('/api/admin/courses/:id/toggle', adminRequired, async (req, res) => {
  ...
  try {
    const database = await getDbConnection();
    const course = await database.get("SELECT is_active FROM courses WHERE id=?", [courseId]);
    if (!course) {
      return res.status(404).json({ error: 'Course not found.' });
    }
    const newStatus = course.is_active ? 0 : 1;
    await database.run("UPDATE courses SET is_active=? WHERE id=?", [newStatus, courseId]);
...
app.get('/sitemap.xml', async (req, res) => {
  try {
    const database = await getDbConnection();
    const courses = await database.all('SELECT id FROM courses ORDER BY id ASC');
...
if (process.env.NODE_ENV === 'production' || fs.existsSync(path.join(__dirname, 'client/dist'))) {
  ...
  app.get('/', (req, res) => {
    res.send('YVN Academy API is running. Start the React dev server to view the frontend!');
  });
}
